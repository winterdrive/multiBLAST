...
Load_file_dir="C:/PYTHON/Download_Seq_files/acclist/20211013_testo_total/"
...
with open(Save_file_dir+"union_testo.csv","w")as file:
    file.write("")
for i in filename:
    column_names = ["", "accession"]
    df = pd.read_csv(Save_file_dir+"union_testo.csv", names=column_names)
    #print(df)
    ...
    result_x.to_csv(Save_file_dir+"union_testo.csv")
...
# 使用月份當做X軸資料
BLASTnumber=list(range(0,881))#這裡的200要隨總檔案數做變換，這次200就田200，下次1000就填1000
x=BLASTnumber
y=seqN
# 設定圖片大小為長15、寬10
